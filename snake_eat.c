//The function of this version is to get the snake move and eat something.
#include <conio.h>
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
int foodX = 1 + rand() % 10;//for the first time place the food randomly
int foodY = 1 + rand() % 10;
...
void put_food(void){
	if (snakeX[snake_head] == foodX && snakeY[snake_head] == foodY) {
        while (true) {
			srand(time(NULL));
            foodX = 1 + rand() % 10;//place the food randomly again
            foodY = 1 + rand() % 10;
            if (map[foodX][foodY] == ' ') {//to make sure the food is placed in a blank place
                break;
            }
        }
    }
